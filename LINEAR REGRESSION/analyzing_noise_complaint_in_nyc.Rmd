...
The analysis uses three datasets to build the model. The first one is NYPD 311 Service Requests from 2010 to Present provided by 311, DoITT and distributed by [NYC Open Data](https://data.cityofnewyork.us/Social-Services/311-Service-Requests-from-2010-to-Present/erm2-nwe9). This dataset contains information about service requests made to the city's 311 service for various issues, including noise complaints related to vehicles . The dataset was filtered to include only noise complaints related to vehicles and was aggregated by zip code to count the number of complaints per zip code.
...
# CLEANING MOTOR_Vehicle_Collision DATA AND SAVING AS vehicle_collision_agg.csv (No need to run anymore as i export a datset from this and use that to mergev)

```{r, echo=FALSE}

# Subset the dataset to only include relevant columns
motor_vehical <- motor_vehical %>%
  select(zip_code, CRASH.DATE, NUMBER.OF.PERSONS.INJURED, NUMBER.OF.PERSONS.KILLED,BOROUGH,NUMBER.OF.PEDESTRIANS.KILLED)

# Convert the crash_date column to a Date format
motor_vehical$crash_date <- as.Date(motor_vehical$CRASH.DATE, format = "%m/%d/%Y")

# Subset the dataset to only include data from the last 5 years
motor_vehical <- motor_vehical %>%
  filter(CRASH.DATE >= as.Date("01-01-2019")) 

# Remove any rows with missing data or invalid zip codes
motor_vehical <- motor_vehical %>%
  filter(!is.na(zip_code)) %>%
  filter(zip_code != "N/A")

# Aggregate the dataset by zip code, calculating the total number of crashes, injuries, and fatalities for each zip code
vehicle_collision_agg <- motor_vehical %>%
  group_by(zip_code) %>%
  summarize(crash_count = n(),
            injured_count = sum(NUMBER.OF.PERSONS.INJURED, na.rm = TRUE),
            killed_count = sum(NUMBER.OF.PERSONS.KILLED, na.rm = TRUE),
            injured_ped_count = sum(NUMBER.OF.PEDESTRIANS.KILLED, na.rm = TRUE))

# Rename the columns
names(vehicle_collision_agg) <- c("zip_code", "crash_count", "injured_count", "killed_count","injured_ped_count")

# Save the cleaned and aggregated dataset as a CSV file
# write.csv(vehicle_collision_agg, "vehicle_collision_agg_2019.csv", row.names = FALSE)

```

# Cleaning the NYC_311 and saving as  Modified_nyc_311_2019.csv (No need to run anymore as i export a datset from this and use that to merge )
```{r, echo=FALSE}
#nyc_service req
#Nyc_311 = read.csv("/Users/rameasaarna/Desktop/assignment 2 /311_Service_Requests_from_2010_to_Present.csv")
...
#NYC_311_data <- Nyc_311 %>%
  #filter(as.numeric(format(as.Date(Created.Date, "%m/%d/%Y"), "%Y")) >= 2019)

## choose the cols 
#Modified_nyc_311 = NYC_311_data%>%
   #select(Created.Date, Complaint.Type,zip_code,City,Borough)%>%
   # filter(!is.na(zip_code)) %>%
    #filter(zip_code != "N/A")

#unique(Nyc_311$zip_code)
# Aggregate the data by zipcode and get a count of the number of requests
#NYC_311_agg_data <- NYC_311_data %>%
  #group_by(zip_code) %>%
  #summarize(num_of_complaints_noise_vehicle = n())



# Save the aggregated data as a CSV file (did not use this)
#write.csv(NYC_311_agg_data, "311_requests_noise_vehicle_by_zipcode.csv", row.names = FALSE)

#saving the cleaned modified_nyc_311 (use this to merge)
#write.csv(Modified_nyc_311, "Modified_nyc_311_2019.csv", row.names = FALSE)
```
...
ggplot(merged_data, aes(x = num_of_complaints_noise_vehicle)) + 
  geom_histogram(fill = "#69b3a2", color = "black", binwidth = 10) +
...
Population had a statistically significant effect on the number of noise complaints related to vehicles, with a coefficient estimate of 0.03009 and a p-value of 0.000632. This means that they are positively related and for every one-unit increase in population, there was a 0.03 increase in noise complaints, holding the crash count constant.
